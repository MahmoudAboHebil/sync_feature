...
import '../../../core/isar_service/collections/table_two_collection.dart';
import '../../../core/isar_service/isar_service.dart';
import '../../../injection_container.dart';
import '../../data/models/table_two_model.dart';
import '../../domain/repository/table_repository.dart';
import '../widgets/sync_button_widget.dart';
import 'home_page.dart';

class TableFourPage extends StatelessWidget {
  const TableFourPage({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          if (message != null && getResult.isRight && tableTwo != null) {
            ...
            final result = await sl<TableFourDatasource>().create(newRecord);
            result.fold(
              ifLeft: (err) {
                MaterialPageRoute(
      ...
      appBar: AppBar(title: Text('Table Four')),
      body: Column(
        ...
        children: [
          ...
          Text(' Table Four data'),
          Expanded(
            child: StreamBuilder<List<TableFourCollection>>(
              stream: IsarService.isar.tableFourCollections
                  .filter()
                  .isDeletedEqualTo(false)
                  .sortByCreatedAtDesc()
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return GestureDetector(
                        ...
                        relationResult.fold(
                          ...
                          ifRight: (relationList) async {
                            ...
                            if (result != null && result) {
                              final model = TableFourModel.fromCollection(
                                record,
                              );
                              final result = await sl<TableFourDatasource>()
                      ...
                      onTap: () async {
                        ...
                        if (message != null) {
                          final result = await sl<TableFourDatasource>().update(
                            TableFourModel.fromCollection(
                              data[i],
