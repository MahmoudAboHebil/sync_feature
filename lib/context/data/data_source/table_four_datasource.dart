...
class TableFourDatasource {
  ...
  Future<Either<Failure, SyncResponse?>> create(TableFour entity) async {
    final uuid = Uuid();
  ...
  Future<Either<Failure, SyncResponse?>> softDelete(TableFour entity) async {
    try {
      final uuid = Uuid();
      final deviceIdResult = await _syncRepository.getDeviceId();
      final deviceId = deviceIdResult.getOrThrow();
      TableFour entityEdited = entity.copyWith(
        isDeleted: true,
        byDevice: deviceId,
        byUser: currentUser,
      ...
      if (!hasInternet) {
        ...
        final deleteRelationResult = await _tableRepository
            .deleteEntityCascadeNotNull(DBTable.table_four, entity.entityId);
        return Right(null);
      } else {
        final sendResult = await _pushSingleOperationUseCase.call(
          PushSingleOperationUseCaseParams(
  ...
  Future<void> _updateEntity<T>(TableFourModel model) async {
    ...
    if (oldCollection != null) {
      ...
      await IsarService.isar.writeTxn(() async {
        final x = await IsarService.isar.tableFourCollections.put(
          newCollection,
        );
  ...
  Future<Either<Failure, SyncResponse?>> update(TableFour newEntity) async {
    try {
      final uuid = Uuid();
      final deviceIdResult = await _syncRepository.getDeviceId();
      final deviceId = deviceIdResult.getOrThrow();

      TableFour entityEdited = newEntity.copyWith(
        byDevice: deviceId,
        byUser: currentUser,
        updatedAt: DateTime.now().toUtc(),
      );
      ...
      if (!hasInternet) {
        ...
        await _updateEntity(TableFourModel.fromEntity(entityEdited));
        return Right(null);
      } else {
        final sendResult = await _pushSingleOperationUseCase.call(
          PushSingleOperationUseCaseParams(
