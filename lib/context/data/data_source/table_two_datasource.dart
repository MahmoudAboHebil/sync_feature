...
class TableTwoDatasource {
  ...
  Future<Either<Failure, SyncResponse?>> create(TableTwo entity) async {
    final uuid = Uuid();
  ...
  Future<Either<Failure, SyncResponse?>> softDelete(TableTwo entity) async {
    try {
      final uuid = Uuid();
      final deviceIdResult = await _syncRepository.getDeviceId();
      final deviceId = deviceIdResult.getOrThrow();
      TableTwo entityEdited = entity.copyWith(
        isDeleted: true,
        byDevice: deviceId,
        byUser: currentUser,
  ...
  Future<void> _updateEntity(TableTwoModel model) async {
    ...
    if (oldCollection != null) {
      final newCollection = TableTwoCollection()
        ..id = oldCollection.id
        ..forKeyTableOne = oldCollection.forKeyTableOne
      ...
      await IsarService.isar.writeTxn(() async {
        final x = await IsarService.isar.tableTwoCollections.put(newCollection);
        print('final update =$x');
      });
    } else {
      throw Exception(
        'there is no record in db for ${model.entityId} at ${DBTable.table_two.name} table',
      );
    }
  }

  Future<Either<Failure, SyncResponse?>> update(TableTwo newEntity) async {
    try {
      final uuid = Uuid();
      final deviceIdResult = await _syncRepository.getDeviceId();
      final deviceId = deviceIdResult.getOrThrow();

      TableTwo entityEdited = newEntity.copyWith(
        byDevice: deviceId,
        byUser: currentUser,
        updatedAt: DateTime.now().toUtc(),
      );
      ...
      if (!hasInternet) {
        ...
        await _updateEntity(TableTwoModel.fromEntity(entityEdited));
        return Right(null);
      } else {
        final sendResult = await _pushSingleOperationUseCase.call(
          PushSingleOperationUseCaseParams(
