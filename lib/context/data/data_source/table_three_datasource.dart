...
class TableThreeDatasource {
  ...
  Future<Either<Failure, SyncResponse?>> create(TableThree entity) async {
    final uuid = Uuid();
  ...
  Future<Either<Failure, SyncResponse?>> softDelete(TableThree entity) async {
    try {
      final uuid = Uuid();
      final deviceIdResult = await _syncRepository.getDeviceId();
      final deviceId = deviceIdResult.getOrThrow();
      TableThree entityEdited = entity.copyWith(
        isDeleted: true,
        byDevice: deviceId,
        byUser: currentUser,
      ...
      if (!hasInternet) {
        ...
        final deleteRelationResult = await _tableRepository
            .deleteEntityCascadeNotNull(DBTable.table_three, entity.entityId);
        return Right(null);
      } else {
        final sendResult = await _pushSingleOperationUseCase.call(
          PushSingleOperationUseCaseParams(
  ...
  Future<void> _updateEntity(TableThreeModel model) async {
    ...
    if (oldCollection != null) {
      final newCollection = TableThreeCollection()
        ..id = oldCollection.id
        ..forKeyTableTwo = oldCollection.forKeyTableTwo
      ...
      await IsarService.isar.writeTxn(() async {
        final x = await IsarService.isar.tableThreeCollections.put(
          newCollection,
        );
      ...
      throw Exception(
        'there is no record in db for ${model.entityId} at ${DBTable.table_three.name} table',
      );
    }
  }

  Future<Either<Failure, SyncResponse?>> update(TableThree newEntity) async {
    try {
      final uuid = Uuid();
      final deviceIdResult = await _syncRepository.getDeviceId();
      final deviceId = deviceIdResult.getOrThrow();

      TableThree entityEdited = newEntity.copyWith(
        byDevice: deviceId,
        byUser: currentUser,
        updatedAt: DateTime.now().toUtc(),
      );
      ...
      if (!hasInternet) {
        ...
        await _updateEntity(TableThreeModel.fromEntity(entityEdited));
        return Right(null);
      } else {
        final sendResult = await _pushSingleOperationUseCase.call(
          PushSingleOperationUseCaseParams(
